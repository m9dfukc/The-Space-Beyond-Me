...
void CAHVFile::setup(string _scene, string _posFile) {
	...
	while( std::getline( data, line ) ) {
		...
		if (pieces.size() == 29) {
			...
			position.set(Lx*multi, Ly*multi, focal/multi);
	...
	for (int i = 0; i < positions.size(); i++) {
		...
		float _x = positions[i].x - _xO;
		float _y = positions[i].y - _yO;
		float _z = (positions[i].z / maxFocal) - _zO ;
		...
		float _x = positions[i+1].x - positions[i].x;
		float _y = positions[i+1].y - positions[i].y;
		float _z = (positions[i+1].z / maxFocal) - (positions[i].z / maxFocal);
		
		positions[i].set(_x, _y, _z);
		
		*/
		
		//cout << "zoom:	" << positions[i].z << "	x: " << positions[i].x << "		y: " << positions[i].y << endl;
	}
