...
namespace sbm {
  ...
  void Sensor::setup(void (*_nextFrame)(), void (*_previousFrame)(), void (*_nextScene)(), void (*_startFilm)(), void (*_endFilm)()){
    ...
    if (Serial1.available() > 0) {
      ...
      if( input == 0 || input == 1 ) {
        if (forward = (input == 0)) {
          ...
          framePos++; 
          if (framePos == frameLastTrigger + 5) nextSceneCounter = 0; // reset nextScene Counter Threshold
          if (framePos == frameLastTrigger + 20) startCounter = 0;
        } else {
          previousFrame();
          framePos--;
          nextSceneCounter = 0; // reset nextScene Counter Threshold
          if (framePos == frameLastTrigger - 20) endCounter = 0;
        } 
